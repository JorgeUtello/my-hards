...
class Server:
    def __init__(self, config: dict):
        ...
        # Last known cursor position (used to compute deltas)
        self._last_x = self._cx
        self._last_y = self._cy
        # Skip-once flag: skip the synthetic event generated by our warp
        self._skip_warp = False
        # Recenter only after the cursor drifts far enough from the anchor.
        # This keeps relay smooth while preventing the server cursor from roaming.
        self._RECENTER_MARGIN = 140
        ...
        # Focus tracking: only intercept input when our app has focus
    ...
    def _switch_to_client(self):
        ...
        # Warp cursor to center; skip the synthetic event it generates
        self._skip_warp = True
        self._last_x = self._cx
        self._last_y = self._cy
    ...
    def _on_mouse_move(self, x: int, y: int):
        ...
        # Skip the single synthetic event generated by the last programmatic warp
        if self._skip_warp:
            self._skip_warp = False
            return

        if self.active:
            # Forward relative movement while the server cursor stays near center.
            dx = x - self._last_x
            dy = y - self._last_y
            ...
            # Recenter only when the cursor drifts too far from the anchor.
            if (
                abs(x - self._cx) >= self._RECENTER_MARGIN
                or abs(y - self._cy) >= self._RECENTER_MARGIN
            ):
                self._skip_warp = True
                self._last_x = self._cx
                self._last_y = self._cy
        ...
        # Not active: detect edge only while the app is focused.
    ...
    def _on_mouse_click(self, x, y, button, pressed):
        if not self.active or not self._app_focused:
            return
